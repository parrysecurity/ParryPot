...
class CommandParser:
    ...
    async def parse(self, command: str) -> ParsedCommand:
        if not command:
            return ParsedCommand(original='', command='', arguments=[], flags=[], has_pipe=False, has_redirect=False, background=False)
        
        original = command.strip()
        background = original.endswith('&')
        has_pipe = '|' in original
        has_redirect = '>' in original or '<' in original
        ...
        return ParsedCommand(
            original=original, command=command_name, arguments=arguments,
            flags=flags, has_pipe=has_pipe, has_redirect=has_redirect, background=background
        )
# Create mitre_mapper.py if missing
cat > honeypot/engine/mitre_mapper.py << 'EOF'
...
class MITREMapper:
    ...
    async def map_attack(self, attack_types: List[str], commands: List[str] = None) -> Dict[str, Any]:
        return {'techniques': [], 'tactics': [], 'matrix_urls': []}
cat > honeypot/main_simple.py << 'EOF'
#!/usr/bin/env python3
...
def main():
    ...
    print(f"Dashboard: http://{HoneypotServer.SERVER_IP}:5000")
    print("=" * 60)
    ...
    for port, name in tcp_ports:
        try:
            ...
            t.start()
            threads.append(t)
            print(f"  ✓ {name} listener on port {port}")
